...
with st.sidebar:
    ...
    st.caption("50,411 rows · 79 columns · 1750-2024")
...
# ── Global CO2 Trend ──────────────────────────────────────────────────────────
    ...
    for yr, label in events.items():
        ...
        if not val.empty:
            ...
            fig.add_annotation(x=yr, y=val.values[0]+2.5, text=label, showarrow=False,
    ...
    fig2 = px.bar(growth, x="year", y="co2_growth_prct", color="co2_growth_prct",
        ...
        title="Year-on-Year CO2 Growth Rate (%)",
...
# ── Top Emitters ──────────────────────────────────────────────────────────────
    ...
    st.markdown('<div class="insight-box">Key Insight: China and USA account for ~40% of global CO2 by volume. Gulf states lead per-capita rankings — a critical policy distinction.</div>', unsafe_allow_html=True)
...
# ── Temperature Impact ────────────────────────────────────────────────────────
    ...
    fig = px.bar(temp.sort_values("temperature_change_from_co2"),
        ...
        color="temperature_change_from_co2", color_continuous_scale=["#00c896","#ffc864","#ff4444"],
    ...
    top5 = temp.head(5)
    fig2 = go.Figure()
    fig2.add_trace(go.Bar(name="CO2",     x=top5["country"], y=top5["temperature_change_from_co2"], marker_color="#00c896"))
    fig2.add_trace(go.Bar(name="Methane", x=top5["country"], y=top5["temperature_change_from_ch4"], marker_color="#ffc864"))
    fig2.add_trace(go.Bar(name="N2O",     x=top5["country"], y=top5["temperature_change_from_n2o"], marker_color="#ff8c69"))
    ...
    st.plotly_chart(fig2, use_container_width=True)
    st.markdown('<div class="insight-box">Key Insight: The USA leads in cumulative warming contribution due to 100+ years of heavy industrial emissions — more than China despite China emitting more today.</div>', unsafe_allow_html=True)
...
# ── Energy Sources ────────────────────────────────────────────────────────────
    ...
    for col, label, color in [("coal_co2","Coal","#ff6b6b"),("oil_co2","Oil","#ffc864"),
        ...
        fig.add_trace(go.Scatter(x=energy["year"], y=energy[col], mode="lines",
    ...
    if selected:
        ...
        st.plotly_chart(fig2, use_container_width=True)
    st.markdown('<div class="insight-box">Key Insight: Coal remains the #1 source globally. The energy transition is deeply uneven — Europe cut coal dramatically while Asia still relies heavily on it.</div>', unsafe_allow_html=True)
...
# ── Emissions vs GDP ──────────────────────────────────────────────────────────
    ...
    fig = px.scatter(sc, x="gdp_per_capita", y="co2_per_capita", size="population",
        ...
        title="CO2 Per Capita vs GDP Per Capita (bubble = population)",
    ...
    st.plotly_chart(fig2, use_container_width=True)
    st.markdown('<div class="insight-box">Key Insight: Richer nations emit more per person. But globally, CO2 intensity has been falling since the 1980s — the economy is becoming more efficient, just not fast enough.</div>', unsafe_allow_html=True)

st.markdown("---")
st.markdown("<div style='text-align:center;color:#3a6b50;font-size:0.8rem;'>Built with Python | Streamlit | Plotly &nbsp;|&nbsp; Data: Our World in Data (CC BY 4.0) &nbsp;|&nbsp; Climate Portfolio 2025</div>", unsafe_allow_html=True)